...
TEST(BlockMatrix, BasicAssertions) {
    ...
    // sizes:                          1  3  1  4  3  4
    vector<uint64_t> rangeToSpan{0, 1, 3, 4, 6, 7, 9};
    vector<set<uint64_t>> columnParams{{0, 1, 2, 5, 8}, {1, 2, 3, 6, 7},
                                       {3, 4, 5, 8},    {4, 5, 7},
                                       {6, 8},          {7, 8}};
    SparseStructure sStruct = columnsToCscStruct(columnParams);
    BlockMatrixSkel skel(spanStart, rangeToSpan, sStruct.ptrs, sStruct.inds);

    ASSERT_THAT(skel.spanStart, ContainerEq(spanStart));
    ASSERT_THAT(skel.rangeToSpan, ContainerEq(rangeToSpan));
    ASSERT_THAT(skel.spanToRange, ElementsAre(0, 1, 1, 2, 3, 3, 4, 5, 5));
    ASSERT_THAT(skel.rangeStart, ElementsAre(0, 1, 4, 5, 9, 12, 16));
    ...
    */
    ...
    ASSERT_THAT(skel.slabRowRange, ElementsAre(0, 1, 3, 5, kInvalid,  //
                                               1, 2, 4, 5, kInvalid,  //
                                               2, 3, 5, kInvalid,     //
                                               3, 5, kInvalid,        //
                                               4, 5, kInvalid,        //
    ...
    ASSERT_THAT(skel.slabColRange, ElementsAre(0,        //
                                               0, 1,     //
                                               1, 2,     //
                                               0, 2, 3,  //
                                               1, 4,     //
