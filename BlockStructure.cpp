...
GroupedBlockStructure::GroupedBlockStructure(const BlockStructure& blockStructure,
    ...
    CHECK_EQ(blockStructure.paramSize.size(), aggregParamStart[aggregParamStart.size()-1]);
    ...
    for(int a = 0; a < aggregParamStart.size() - 1; a++) {
	    ...
	    for(uint64_t p = pStart; p < pEnd; p++) {
		    allRows.insert(blockStructure.colBlocks[p].begin(), blockStructure.colBlocks[p].end());
	    }
