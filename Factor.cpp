...
std::pair<uint64_t, uint64_t> findBlock(const BlockMatrixSkel& skel, uint64_t cParam, uint64_t rParam) {
	uint64_t aggreg = skel.paramToAggreg[cParam];
	uint64_t aggregSize = skel.aggregStart[aggreg + 1] - skel.aggregStart[aggreg];
	uint64_t offsetInAggreg = skel.paramStart[cParam] - skel.aggregStart[aggreg];
	...
	uint64_t pos = bisect(skel.blockRowParam.data() + start, end - start, rParam);
	CHECK_EQ(skel.blockRowParam[start + pos], rParam);
	return std::make_pair(skel.blockData[start + pos] + offsetInAggreg, aggregSize);
}

using OuterStride = Eigen::OuterStride<>;
using OuterStridedMatM = Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>, 0, OuterStride>;
...
void eliminateAggregItem(const BlockMatrixSkel& skel,
	...
	uint64_t gatheredStart = skel.blockColGatheredDataPtr[aggreg];
	uint64_t gatheredEnd = skel.blockColGatheredDataPtr[aggreg+1];
	uint64_t rowDataStart = skel.blockRowAggregParamPtr[gatheredStart + rowItem];
	uint64_t rowDataEnd0 = skel.blockRowAggregParamPtr[gatheredStart + rowItem + 1];
	...
	uint64_t numRowsSub = skel.endBlockNumRowsAbove[colStart + rowDataEnd0 - 1] - rowStart;
	uint64_t numRowsFull = skel.endBlockNumRowsAbove[colStart + rowDataEnd1 - 1] - rowStart;
	
	//LOG(INFO) << "aggreg: " << aggreg << ", item: " << rowItem;
	//LOG(INFO) << "sizes: " << numRowsSub << " " << numRowsFull << " " << aggregSize;
	Eigen::Map<MatRMaj<double>> belowDiagBlockSub(data.data() + belowDiagStart, numRowsSub, aggregSize);
	Eigen::Map<MatRMaj<double>> belowDiagBlockFull(data.data() + belowDiagStart, numRowsFull, aggregSize);
	...
	for(uint64_t c = rowDataStart; c < rowDataEnd0; c++) {
		uint64_t cStart = skel.endBlockNumRowsAbove[colStart + c - 1] - rowStart;
		uint64_t cSize = skel.endBlockNumRowsAbove[colStart + c] - cStart - rowStart;
		uint64_t cParam = skel.blockRowParam[colStart + c];
		for(uint64_t r = rowDataStart; r < rowDataEnd1; r++) {
			uint64_t rStart = skel.endBlockNumRowsAbove[colStart + r - 1] - rowStart;
			uint64_t rSize = skel.endBlockNumRowsAbove[colStart + r] - rStart - rowStart;
			uint64_t rParam = skel.blockRowParam[colStart + r];
			//LOG(INFO) << rStart << ":" << rSize << "@" << rParam << " x " << cStart << ":" << cSize << "@" << cParam;

			auto [offset, stride] = findBlock(skel, cParam, rParam);
			OuterStridedMatM target(data.data() + offset, rSize, cSize, OuterStride(stride));
