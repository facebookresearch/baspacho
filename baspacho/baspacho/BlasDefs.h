...
void dpotrf_(const char* uplo, BLAS_INT* n, double* A, const BLAS_INT* lda,
             BLAS_INT* info);
...
void dgemm_(const char* transa, const char* transb, const BLAS_INT* m,
...
void dsymm_(const char* uplo, const char* transa, const BLAS_INT* m,
...
void spotrf_(const char* uplo, BLAS_INT* n, float* A, const BLAS_INT* lda,
             BLAS_INT* info);

void strsm_(const char* side, const char* uplo, const char* transa,
            const char* diag, const BLAS_INT* m, const BLAS_INT* n,
            const float* alpha, const float* A, const BLAS_INT* lda, float* B,
            const BLAS_INT* ldb);

void sgemm_(const char* transa, const char* transb, const BLAS_INT* m,
            const BLAS_INT* n, const BLAS_INT* k, const float* alpha,
            const float* A, const BLAS_INT* lda, const float* B,
            const BLAS_INT* ldb, const float* beta, float* C,
            const BLAS_INT* ldc);

void ssyrk_(const char* uplo, const char* transa, const BLAS_INT* n,
            const BLAS_INT* k, const float* alpha, const float* A,
            const BLAS_INT* lda, const float* beta, float* C,
            const BLAS_INT* ldc);

void ssymm_(const char* uplo, const char* transa, const BLAS_INT* m,
...
inline void cblas_dsymm(const CBLAS_LAYOUT /* Layout */, const CBLAS_SIDE side,
                        const CBLAS_UPLO uplo, const BLAS_INT m,
                        const BLAS_INT n, const double alpha, const double* a,
                        const BLAS_INT lda, const double* b, const BLAS_INT ldb,
                        const double beta, double* c, const BLAS_INT ldc) {
