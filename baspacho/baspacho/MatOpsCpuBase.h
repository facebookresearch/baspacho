...
struct CpuBaseSymbolicCtx : SymbolicCtx {
  ...
  static int64_t computeMaxBufSize(const CpuBaseSymElimCtx& elim,
    ...
    for (int64_t i = elim.rowPtr[sRel], iEnd = elim.rowPtr[sRel + 1]; i < iEnd;
         i++) {
  ...
  virtual SymElimCtxPtr prepareElimination(int64_t lumpsBegin,
    ...
    for (int64_t l = lumpsBegin; l < lumpsEnd; l++) {
      for (int64_t i = skel.chainColPtr[l], iEnd = skel.chainColPtr[l + 1];
           i < iEnd; i++) {
    ...
    for (int64_t l = lumpsBegin; l < lumpsEnd; l++) {
      for (int64_t iBegin = skel.chainColPtr[l], iEnd = skel.chainColPtr[l + 1],
                   i = iBegin;
           i < iEnd; i++) {
        int64_t s = skel.chainRowSpan[i];
        if (s < spanRowBegin) {
    ...
    for (int64_t r = 0; r < (int64_t)elim->rowPtr.size() - 1; r++) {
      elim->maxBufferSize =
          std::max(elim->maxBufferSize, computeMaxBufSize(*elim, skel, r));
    }
...
struct CpuBaseNumericCtx : NumericCtx<T> {
  ...
  static void eliminateRowChain(const CpuBaseSymElimCtx& elim,
    ...
    for (int64_t i = elim.rowPtr[sRel], iEnd = elim.rowPtr[sRel + 1]; i < iEnd;
      ...
      T* headTargetData =
          data + spanOffsetInLump + pSpanToChainOffset[pChainRowSpan[ptrStart]];
      T* nextDataStart = elimDiagBlock(nRowsChain, lumpSize, headTargetData,
      ...
      for (int64_t ptr = ptrStart + 1; ptr < ptrEnd; ptr++) {
        ...
        nextDataStart = elimBlock(s2_size, nRowsChain, lumpSize, targetData,
  ...
  static void eliminateVerySparseRowChain(const CpuBaseSymElimCtx& elim,
    ...
    for (int64_t i = elim.rowPtr[sRel], iEnd = elim.rowPtr[sRel + 1]; i < iEnd;
      ...
      Eigen::Map<MatRMaj<T>> chainSubMat(data + dataOffset, nRowsChain,
                                         lumpSize);
      Eigen::Map<MatRMaj<T>> chainOnwardSubMat(data + dataOffset, nRowsOnward,
                                               lumpSize);
      ...
      for (int64_t ptr = ptrStart; ptr < ptrEnd; ptr++) {
        ...
        stridedMatSub(targetData, targetLumpSize,
                      tempBuffer + nRowsChain * relRow, nRowsChain, s2_size,
                      nRowsChain);
      }
    }
  }
