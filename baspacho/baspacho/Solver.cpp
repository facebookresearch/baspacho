...
Solver::Solver(CoalescedBlockMatrixSkel&& factorSkel_, std::vector<int64_t>&& elimLumpRanges_,
  ...
  for (int64_t l = 0; l + 1 < (int64_t)elimLumpRanges.size(); l++) {
    elimCtxs.push_back(symCtx->prepareElimination(elimLumpRanges[l], elimLumpRanges[l + 1]));
  }
...
void Solver::initElimination() {
  int64_t denseOpsFromLump = elimLumpRanges.size() ? elimLumpRanges[elimLumpRanges.size() - 1] : 0;
...
template <typename T>
void Solver::internalFactorRange(T* data, int64_t startParamIndex, int64_t endParamIndex,
                                 bool verbose) const {
  ...
  for (int64_t l = 0; l + 1 < (int64_t)elimLumpRanges.size(); l++) {
    ...
    if (verbose) {
      std::cout << "Elim set: " << l << " (" << elimLumpRanges[l] << ".." << elimLumpRanges[l + 1]
                << ")" << std::endl;
    }
    numCtx->doElimination(*elimCtxs[l], data, elimLumpRanges[l], elimLumpRanges[l + 1]);
  }
...
void Solver::solve(const T* matData, T* vecData, int64_t stride, int nRHS) const {
  ...
  internalSolveLRange(*slvCtx, matData, 0, factorSkel.numSpans(), vecData, stride);
  internalSolveLtRange(*slvCtx, matData, 0, factorSkel.numSpans(), vecData, stride);
}
...
template <typename T>
void Solver::solveLUpTo(const T* matData, int64_t paramIndex, T* vecData, int64_t stride,
                        int nRHS) const {
  SolveCtxPtr<T> slvCtx = symCtx->createSolveCtx<T>(nRHS, matData);
...
template <typename T>
void Solver::solveLtUpTo(const T* matData, int64_t paramIndex, T* vecData, int64_t stride,
                         int nRHS) const {
  SolveCtxPtr<T> slvCtx = symCtx->createSolveCtx<T>(nRHS, matData);
...
template <typename T>
void Solver::solveLFrom(const T* matData, int64_t paramIndex, T* vecData, int64_t stride,
                        int nRHS) const {
  SolveCtxPtr<T> slvCtx = symCtx->createSolveCtx<T>(nRHS, matData);
  internalSolveLRange(*slvCtx, matData, paramIndex, factorSkel.numSpans(), vecData, stride);
}

template <typename T>
void Solver::solveLtFrom(const T* matData, int64_t paramIndex, T* vecData, int64_t stride,
                         int nRHS) const {
  SolveCtxPtr<T> slvCtx = symCtx->createSolveCtx<T>(nRHS, matData);
  internalSolveLtRange(*slvCtx, matData, paramIndex, factorSkel.numSpans(), vecData, stride);
}

static constexpr bool SparseElimSolve = true;

template <typename T>
void Solver::internalSolveLRange(SolveCtx<T>& slvCtx, const T* matData, int64_t startParamIndex,
  ...
  if (SparseElimSolve) {
    for (int64_t l = 0; l + 1 < (int64_t)elimLumpRanges.size(); l++) {
      ...
      slvCtx.sparseElimSolveL(*elimCtxs[l], matData, elimLumpRanges[l], elimLumpRanges[l + 1],
                              vecData, stride);
    }

    denseOpsFromLump =
        std::max(startLump, (int64_t)(elimLumpRanges.empty() ? 0 : elimLumpRanges.back()));
  } else {
...
template <typename T>
void Solver::internalSolveLtRange(SolveCtx<T>& slvCtx, const T* matData, int64_t startParamIndex,
                                  int64_t endParamIndex, T* vecData, int64_t stride) const {
  BASPACHO_CHECK_GE(startParamIndex, 0);
  BASPACHO_CHECK_LE(startParamIndex, endParamIndex);
  BASPACHO_CHECK_LT(endParamIndex, (int64_t)factorSkel.spanOffsetInLump.size());
  ...
  if (SparseElimSolve) {
    for (int64_t l = (int64_t)elimLumpRanges.size() - 2; l >= 0; l--) {
      ...
      slvCtx.sparseElimSolveLt(*elimCtxs[l], matData, elimLumpRanges[l], elimLumpRanges[l + 1],
                               vecData, stride);
    }
  }
}
...
template <typename T>
void Solver::addMvFrom(const T* matData, int64_t paramIndex, const T* inVecData, int64_t inStride,
...
template <typename T>
void Solver::pseudoFactorFrom(T* data, int64_t paramIndex, bool /* verbose */) const {
  NumericCtxPtr<T> numCtx = symCtx->createNumericCtx<T>(maxElimTempSize, data);
...
template void Solver::solveLt<vector<float*>>(const vector<float*>* matData,
                                              ...
                                              int nRHS) const;
template void Solver::factorUpTo<double>(double* data, int64_t paramIndex, bool verbose) const;
template void Solver::factorUpTo<float>(float* data, int64_t paramIndex, bool verbose) const;
template void Solver::factorUpTo<vector<double*>>(vector<double*>* data, int64_t paramIndex,
                                                  bool verbose) const;
template void Solver::factorUpTo<vector<float*>>(vector<float*>* data, int64_t paramIndex,
                                                 bool verbose) const;
template void Solver::factorFrom<double>(double* data, int64_t paramIndex, bool verbose) const;
template void Solver::factorFrom<float>(float* data, int64_t paramIndex, bool verbose) const;
template void Solver::factorFrom<vector<double*>>(vector<double*>* data, int64_t paramIndex,
                                                  bool verbose) const;
template void Solver::factorFrom<vector<float*>>(vector<float*>* data, int64_t paramIndex,
                                                 bool verbose) const;
template void Solver::solveLUpTo<double>(const double* matData, int64_t paramIndex, double* vecData,
                                         int64_t stride, int nRHS) const;
template void Solver::solveLUpTo<float>(const float* matData, int64_t paramIndex, float* vecData,
                                        int64_t stride, int nRHS) const;
template void Solver::solveLUpTo<vector<double*>>(const vector<double*>* matData,
                                                  int64_t paramIndex, vector<double*>* vecData,
                                                  int64_t stride, int nRHS) const;
template void Solver::solveLUpTo<vector<float*>>(const vector<float*>* matData, int64_t paramIndex,
                                                 vector<float*>* vecData, int64_t stride,
                                                 int nRHS) const;
template void Solver::solveLtUpTo<double>(const double* matData, int64_t paramIndex,
                                          double* vecData, int64_t stride, int nRHS) const;
template void Solver::solveLtUpTo<float>(const float* matData, int64_t paramIndex, float* vecData,
                                         int64_t stride, int nRHS) const;
template void Solver::solveLtUpTo<vector<double*>>(const vector<double*>* matData,
                                                   int64_t paramIndex, vector<double*>* vecData,
                                                   int64_t stride, int nRHS) const;
template void Solver::solveLtUpTo<vector<float*>>(const vector<float*>* matData, int64_t paramIndex,
                                                  vector<float*>* vecData, int64_t stride,
                                                  int nRHS) const;
template void Solver::addMvFrom<double>(const double* matData, int64_t paramIndex,
...
template void Solver::solveLFrom<double>(const double* matData, int64_t paramIndex, double* vecData,
                                         int64_t stride, int nRHS) const;
template void Solver::solveLFrom<float>(const float* matData, int64_t paramIndex, float* vecData,
                                        int64_t stride, int nRHS) const;
template void Solver::solveLtFrom<double>(const double* matData, int64_t paramIndex,
                                          double* vecData, int64_t stride, int nRHS) const;
template void Solver::solveLtFrom<float>(const float* matData, int64_t paramIndex, float* vecData,
                                         int64_t stride, int nRHS) const;
...
static SolverPtr createSolverQ(const Settings& settings, const std::vector<int64_t>& paramSize,
  ...
  return SolverPtr(new Solver(move(factorSkel), move(et.sparseElimRanges), move(etTotalInvPerm),
                              et.permInverse.size(), getBackend(settings)));
}

SolverPtr createSolver(const Settings& settings, const std::vector<int64_t>& paramSize,
                       const SparseStructure& ss_, const std::vector<int64_t>& elimLumpRanges,
  ...
  if (settings.addFillPolicy == AddFillNone || settings.addFillPolicy == AddFillForGivenElims) {
    ...
    return SolverPtr(new Solver(move(factorSkel), std::move(elimLumpRangesCopy), move(permutation),
  ...
  shiftConcat(fullLumpToSpan, elimEnd, et.lumpToSpan.begin(), et.lumpToSpan.end());
  BASPACHO_CHECK_EQ((int64_t)fullSpanStart.size() - 1, fullLumpToSpan[fullLumpToSpan.size() - 1]);
  ...
  BASPACHO_CHECK_EQ((int64_t)fullRowParam.size(), fullColStart[fullColStart.size() - 1]);
  ...
  int64_t autoElimEnd = fullElimLumpRanges.empty() ? 0 : fullElimLumpRanges.back();

  return SolverPtr(
      new Solver(move(factorSkel), move(fullElimLumpRanges), move(fullInvPerm),
                 settings.addFillPolicy == AddFillForAutoElims ? autoElimEnd : paramSize.size(),
                 getBackend(settings)));
}
