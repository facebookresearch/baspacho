...
static std::ostream& operator<<(std::ostream& os, const ba_at_large::Data::Obs& obs) {
  return os << "Obs[c:" << obs.camIdx << ",p:" << obs.ptIdx << ",at:(" << obs.imgPos.x() << ","
            << obs.imgPos.y() << ")]";
}

static std::ostream& operator<<(std::ostream& os, const ba_at_large::Data::Cam& cam) {
  Quat rot = cam.T_W_C.so3().unit_quaternion();
  Vec3 tr = cam.T_W_C.translation();
  return os << "Cam[r:q(" << rot.w() << "," << rot.x() << "," << rot.y() << "," << rot.z() << ")"
            << ",t:(" << tr.x() << "," << tr.y() << "," << tr.z() << ")"
            << ",f:" << cam.f_k1_k2[0] << ",k1:" << cam.f_k1_k2[1] << ",k2:" << cam.f_k1_k2[2]
            << "]";
}

static std::ostream& operator<<(std::ostream& os, const ba_at_large::Vec3& vec3) {
...
void Data::removeBadObservations(int64_t maxNumPts) {
  ...
  cout << "num Obs: " << observations.size() << " -> " << newObservations.size() << endl;
...
void Data::load(const std::string& path, bool verbose) {
  ...
  for (int i = 0; i < num_observations; i++) {
    ...
    if (camIdx >= num_cameras || camIdx < 0) {
      throw runtime_error("Error loading " + to_string(i) +
    ...
    if (!istream) {
      throw runtime_error("File error loading " + to_string(i) + "th observation!");
    }
  }
  ...
  for (int i = 0; i < num_cameras; i++) {
    ...
    istream >> rRot[0] >> rRot[1] >> rRot[2] >> tr[0] >> tr[1] >> tr[2] >> f >> k1 >> k2;
...
void Data::save(const std::string& path, bool verbose) {
  ...
  ostream << cameras.size() << " " << points.size() << " " << observations.size() << endl;

  for (Obs& obs : observations) {
    ostream << obs.camIdx << " " << obs.ptIdx << " " << obs.imgPos.x() << " " << obs.imgPos.y()
            << endl;
  }
...
void Data::compute_fill_stats() const {
  ...
  cout << "connected_pairs: " << cpairs << " / " << (ncams * (ncams - 1) / 2)
       << " (n.cams=" << ncams << ")" << endl;
  cout << "S matrix filling: " << double(cpairs * 2 + ncams) / (ncams * ncams) << endl;
}
