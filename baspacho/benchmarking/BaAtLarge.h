...
struct Data {
    ...
    struct Cam {
        ...
        Cam(const Quat& _rot, const Vec3& _tr, double _f, double _k1,
            double _k2)
...
struct Cost {
    ...
    static Vec2 compute_residual(const Vec2& imagePt, const Vec3& worldPt,
        ...
        if (point_jacobian) {
            ...
            DprojPt.row(0) =
                (camPt[2] * R.row(0) - camPt[0] * R.row(2)) * denum;
            DprojPt.row(1) =
                (camPt[2] * R.row(1) - camPt[1] * R.row(2)) * denum;

            Eigen::Matrix<double, 1, 3> DpPtSqN =
                2.0 * projPt.transpose() * DprojPt;
            J = (f * r) * DprojPt +
                projPt * DpPtSqN * (f * (k1 + k2 * 2.0 * projPtSqN));
        }
        ...
        if (camera_jacobian) {
            ...
            Eigen::Matrix<double, 1, 6> DpPtSqN =
                2.0 * projPt.transpose() * DprojPt;
            J = (f * r) * DprojPt +
                projPt * DpPtSqN * (f * (k1 + k2 * 2.0 * projPtSqN));
        }
        ...
        if (calib_jacobian) {
            ...
            J.col(2) =
                (f * projPtSqN * projPtSqN) * projPt;  // d(calibPt) / d(k2)
        }
