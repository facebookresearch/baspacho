...
struct Optimizer {
    Optimizer(Data& data) : data(data) {
        ...
        SparseStructure blockStructure =
            columnsToCscStruct(colBlocks).transpose();
    ...
    double computeStep(double lambda) {
        ...
        for (auto& obs : data.observations) {
            ...
            grad.segment<3>(accessor.paramStart(ptId)) +=
                ptJac.transpose() * err;
            grad.segment<6>(accessor.paramStart(camId)) +=
                camJac.transpose() * err;
        }
        ...
        cout << fixed << setprecision(3) << "  <timings - costs: "
             ...
             << "s), solve: " << tdelta(now - startSolve).count() << "s>"
             << endl;
    ...
    void optimize() {
        ...
        for (int i = 0, lastFailed = 0, lastOk = 0, lastGood = 0; i < 50; i++) {
            ...
            cout << "[" << i << "] cost: " << scientific << setprecision(3)
                 << cost << " -> " << newCost
                 << (good  ? "!"
                     : bad ? "?"
                           : "~");
            ...
            cout << " -> " << lambda << ", model-red: " << setprecision(2)
                 << modelRed << fixed << ", rel-red: " << relRed << endl;
