...
CholmodBenchResults benchmarkCholmodSolve(const vector<int64_t>& paramSize,
  ...
  if (verbose >= 1) {
    std::cout << "Matrix stats:"
              << "\n  A size: " << A.ncol << "\n  nz: " << A.nzmax
              << "\n  fill: " << A.nzmax / ((double)A.ncol * A.nrow) << std::endl;
    if (!cholmodFactor_->is_super) {
      std::cout << "Cholmod, simplicial factor nz: " << cholmodFactor_->nzmax << " ("
                << cholmodFactor_->nzmax / ((double)cholmodFactor_->n * cholmodFactor_->n)
                << " fill)" << std::endl;
    } else {
      std::cout << "Node stats:"
                ...
                << cholmodFactor_->xsize / ((double)cholmodFactor_->n * cholmodFactor_->n)
                << " fill)" << std::endl;
  ...
  switch (cc_.status) {
    ...
    case CHOLMOD_TOO_LARGE:
      std::cerr << "CHOLMOD failure: Integer overflow occured." << std::endl;
      exit(1);
    case CHOLMOD_INVALID:
      std::cerr << "CHOLMOD failure: Invalid input." << std::endl;
      exit(1);
    case CHOLMOD_NOT_POSDEF:
      std::cerr << "CHOLMOD warning: Matrix not positive definite." << std::endl;
      exit(1);
    case CHOLMOD_DSMALL:
    ...
    case CHOLMOD_OK:
      ...
      exit(1);
    default:
      std::cerr << "Unknown cholmod return code: " << cc_.status << std::endl;
      exit(1);
  }
