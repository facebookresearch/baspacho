...
SparseMatGenerator SparseMatGenerator::genFlat(int64_t size, double fill, int64_t seed) {
...
SparseMatGenerator SparseMatGenerator::genLine(int64_t size, double fill, int64_t bandSize,
                                               int64_t seed) {
  SparseMatGenerator retv(size, seed);
  retv.connectRanges(0, size, 0, size, fill);
  return retv;
...
SparseMatGenerator SparseMatGenerator::genMeridians(int64_t num, int64_t lineLen, double fill,
                                                    int64_t bandSize, int64_t hairLen,
                                                    int64_t nPoleHairs, int64_t sPoleHairs,
                                                    int64_t seed) {
  ...
  for (int64_t i = 0; i < num; i++) {
    ...
    retv.connectRanges(iBegin, iBegin + lineLen, iBegin, iBegin + lineLen, fill, bandSize);
  }
  for (int64_t h = 0; h < totHairs; h++) {
    int64_t hBegin = endMeridians + hairLen * h;
    retv.connectRanges(hBegin, hBegin + hairLen, hBegin, hBegin + hairLen, fill, bandSize);
  }
  ...
  for (int64_t i = 0; i < num; i++) {
    ...
    for (int64_t j = 0; j < i; j++) {
      ...
      retv.connectRanges(iBegin + lineLen - bandSize, iBegin + lineLen,  //
...
SparseMatGenerator SparseMatGenerator::genGrid(int64_t width, int64_t height, double fill,
                                               int64_t connMaxDist, int64_t seed) {
