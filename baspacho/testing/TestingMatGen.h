...
struct SparseMatGenerator {
  ...
  static SparseMatGenerator genFlat(int64_t size, double fill, int64_t seed = 37);

  // topology is roughly a line, entries in band are set with a probability
  static SparseMatGenerator genLine(int64_t size, double fill, int64_t bandSize, int64_t seed = 37);

  // topology is a set of meridians (connecting north and south poles)
  static SparseMatGenerator genMeridians(int64_t num, int64_t lineLen, double fill,
                                         int64_t bandSize, int64_t hairLen, int64_t nPoleHairs,
                                         int64_t sPoleHairs, int64_t seed = 37);

  // generates parameters in a grid, with connection to neighbors
  static SparseMatGenerator genGrid(int64_t width, int64_t height, double fill, int64_t connMaxDist,
                                    int64_t seed = 37);
