...
TEST(Accessor, CoalescedAccessor) {
  ...
  for (int64_t c = 0; c < colBlocks.size(); c++) {
    ...
    for (int64_t r : colBlocks[c]) {
      ...
      Eigen::Map<Eigen::MatrixXd> randomBlock(randomBlockData.data(), rSize,
                                              cSize);
    ...
    // diagonal
    {
      ...
      Eigen::Map<Eigen::MatrixXd> randomBlock(randomBlockData.data(), cSize,
                                              cSize);
  ...
  ASSERT_NEAR(
      Eigen::MatrixXd((denseMat - densifiedMat).triangularView<Eigen::Lower>())
          .norm(),
      0, 1e-5);
}
