...
using OuterStridedRMajMatM =
    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>, 0,
               OuterStride>;
...
TEST(Accessor, CoalescedAccessor) {
  ...
  for (int64_t c = 0; c < colBlocks.size(); c++) {
    ...
    for (int64_t r : colBlocks[c]) {
      ...
      std::vector<double> randomBlockData = randomData(rSize * cSize, -1.0, 1.0, seed++);
      Eigen::Map<Eigen::MatrixXd> randomBlock(randomBlockData.data(), rSize, cSize);

      auto [off, stride] = acc.blockOffset(r, c);
      OuterStridedRMajMatM block(data.data() + off, rSize, cSize, OuterStride(stride));
      block = randomBlock;
    ...
    // diagonal
    {
      std::vector<double> randomBlockData = randomData(cSize * cSize, -1.0, 1.0, seed++);
      Eigen::Map<Eigen::MatrixXd> randomBlock(randomBlockData.data(), cSize, cSize);

      auto [off, stride] = acc.blockOffset(c, c);
      OuterStridedRMajMatM block(data.data() + off, cSize, cSize, OuterStride(stride));
      block += randomBlock;
  ...
  ASSERT_NEAR(Eigen::MatrixXd((denseMat - densifiedMat).triangularView<Eigen::Lower>()).norm(), 0,
              1e-5);
}
  ...
  for (int64_t pc = 0; pc < colBlocks.size(); pc++) {
    ...
    for (int64_t pr : colBlocks[pc]) {
      ...
      if (!flip) {
        ...
        std::vector<double> randomBlockData = randomData(rSize * cSize, -1.0, 1.0, seed++);
        Eigen::Map<Eigen::MatrixXd> randomBlock(randomBlockData.data(), cSize, rSize);

        OuterStridedCMajMatM block(data.data() + off, cSize, rSize, OuterStride(stride));
        block = randomBlock;
  ...
  for (int64_t pc = 0; pc < colBlocks.size(); pc++) {
    ...
    for (int64_t pr : colBlocks[pc]) {
      ...
      if (!flip) {
        ...
        denseMat.block(cStart, rStart, cSize, rSize) = randomBlock.transpose();
      }
    }
