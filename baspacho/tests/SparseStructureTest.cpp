...
TEST(SparseStructure, SymPermutation) {
  /*
    ...
    X X _ _ X X
  */
  SparseStructure ss({0, 1, 2, 4, 6, 8, 12}, {0, 1, 0, 1, 1, 3, 2, 4, 0, 1, 4, 5});
  ...
  /*
    ...
    _ X X X _ X
  */
  SparseStructure pCsr = ss.symmetricPermutation({4, 5, 2, 1, 0, 3}, /* lowerHalf = */ false);

  ASSERT_THAT(pCsr.ptrs, ElementsAre(0, 1, 2, 3, 5, 8, 12));
  ASSERT_THAT(pCsr.inds, ElementsAre(0, 1, 0, 0, 3, 2, 3, 4, 1, 2, 3, 5));

  SparseStructure pCsc = ss.symmetricPermutation({4, 5, 2, 1, 0, 3}, /* lowerHalf = */ true);
    ...
    for (auto fill : fills) {
      ...
      for (int start = 0; start < size * 2 / 3; start += 3) {
        for (int end = start + 3; end < size; end += 3) {
          vector<set<int64_t>> cols = makeIndependentElimSet(colsOrig, start, end);
...
TEST(SparseStructure, FillReducingPermutation) {
  vector<int64_t> ptrs{0,  9,  15,  21,  27,  33,  39,  48,  57,  61,  70,  76, 82,
                       88, 94, 100, 106, 110, 119, 128, 137, 143, 152, 156, 160};
  vector<int64_t> inds{/* column  0: */ 0, 5,  6,  12, 13, 17, 18, 19, 21,
  ...
  SparseStructure ssOrig = SparseStructure(ptrs, inds).clear();  // lower half csr

  vector<int64_t> permutation = ssOrig.fillReducingPermutation();
  vector<int64_t> invPerm = inversePermutation(permutation);

  SparseStructure ss = ssOrig.symmetricPermutation(invPerm, false).addFullEliminationFill();
